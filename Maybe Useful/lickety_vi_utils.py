...
def get_model_reliances(
    ...
    original_preds = trie.get_all_predictions(X_bool, stack=True).astype(np.uint8)
    original_acc = (original_preds == y_all).mean(axis=1)
    
    perturbed_acc = 0
    for _ in range(num_perts):
        x_perturbed, y_perturbed = perturb_divided(data_df.to_numpy(), var_of_interest)
        Xp_bool = (x_perturbed != 0)
        pert_preds = trie.get_all_predictions(Xp_bool, stack=True).astype(np.uint8)
        perturbed_acc += (pert_preds == y_perturbed).mean(axis=1)
